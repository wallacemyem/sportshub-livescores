    ...
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-surface border border-surface-border rounded-2xl w-full max-w-lg overflow-hidden shadow-elevated animate-in fade-in zoom-in-95 duration-150">
        ...
        <form onSubmit={handleImport} className="p-4 space-y-4">
          {/* Error Notification */}
          {error && (
            <div className="p-3 bg-surface-subtle border border-surface-border text-foreground rounded-xl text-xs flex items-start gap-2.5">
              <AlertCircle className="w-4 h-4 text-foreground shrink-0 mt-0.5" />
              <div>
                <p className="font-bold">Ticket Resolution Failed</p>
                <p className="mt-0.5 text-muted-foreground leading-relaxed">{error}</p>
              </div>
            </div>
          )}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              ...
              {[
                ...
                >
                  ...
                  {bookmaker === b.id && <Check className="w-3 h-3 text-foreground" />}
                </button>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-surface-subtle border border-surface-border focus:border-foreground rounded-lg px-3 py-2 text-sm text-foreground font-mono placeholder:text-muted-foreground focus:outline-none transition-colors"
              required
            />
          </div>
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {SAMPLE_CODES.map((s) => (
                <button
                  ...
                  className="text-left bg-surface-subtle hover:bg-surface-hover border border-surface-border px-2 py-1.5 rounded text-[10px] text-muted-foreground hover:text-foreground transition-colors truncate cursor-pointer font-mono"
                >
                  <strong className="text-foreground">{s.code}</strong> - Auto Loop
                </button>
              ))}
            </div>
          </div>
          ...
          {isLoading && scanStep && (
            <div className="p-2.5 bg-surface-subtle border border-surface-border rounded-lg text-center text-xs font-mono text-foreground flex items-center justify-center gap-2">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full py-2.5 px-4 bg-foreground hover:opacity-90 text-background font-bold text-xs uppercase tracking-wider rounded-lg flex items-center justify-center gap-2 transition-all disabled:opacity-50 cursor-pointer"
          >
            {isLoading ? (
              <>
